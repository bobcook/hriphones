...
Telephony::OFFICE_HOURS = Class.new do
  def self.open?
    case Time.now.utc.hour
      when 11..23 then true
      else false
    end
  end

  def self.open_hours_audio_files(params)
    out = ["http://hriphones.com/StayOn.mp3"]
    out.unshift "http://hriphones.com/StayOn.mp3" if params[:play_zestcash_transition].present?
    out
  end

  def self.closed_hours_audio_files(params)
    ["http://hriphones.com/Closed.mp3"]
  end
